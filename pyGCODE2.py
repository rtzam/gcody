...
class gcode():
    ...
    def header(self):
        ...
        self.comment('This is GCODE generated with pyGCODE version {}'.format(VERSION))
    ...
    def view(self, backend='matplotlib',*args, **kwargs):
...
## Testing ----------------------------------------------------------------------------
def back_n_forth(cycles=10, cycle_d=10, **kwargs):

    # creating gcode object
    g = gcode()

    # creates a header for the gcode file
    g.header()

    # begins back and-forth for number of cycles
    g.move(cycle_d, **kwargs) # moves the print head back and forth in x

    # moves the print head back and forth in x
    for i in range(1,cycles):
        g.simple_move(y=10*i)
        g.simple_move((-1)**i * cycle_d)
...
a = back_n_forth(speed=50, com='Iterating')
